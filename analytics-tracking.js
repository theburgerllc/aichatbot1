...
function detectIndustry() {
    ...
    if (urlParams.get('industry')) {
        trackEvent('industry_detected', { method: 'url_param', industry: urlParams.get('industry') });
    ...
    if (referrer.includes('healthcare') || referrer.includes('medical') || referrer.includes('doctor')) {
        trackEvent('industry_detected', { method: 'referrer', industry: 'healthcare' });
        return 'healthcare';
    }
    ...
    if (referrer.includes('shop') || referrer.includes('ecommerce') || referrer.includes('store') || referrer.includes('shopify')) {
        trackEvent('industry_detected', { method: 'referrer', industry: 'ecommerce' });
        return 'ecommerce';
    }
    ...
    if (getDeviceCategory() === 'mobile' && Math.random() > 0.4) {
        trackEvent('industry_detected', { method: 'device_based', industry: 'ecommerce' });
        return 'ecommerce';
    }
    ...
    if (previousIndustry) {
        trackEvent('industry_detected', { method: 'previous_session', industry: previousIndustry });
...
function classifyIntent() {
    const highIntentIndicators = [
        'pricing', 'demo', 'trial', 'buy', 'purchase', 'signup', 'start',
        'calculator', 'roi', 'savings', 'cost'
    ];
    
    const mediumIntentIndicators = [
        'features', 'how-it-works', 'benefits', 'compare', 'vs',
    ...
    if (mediumIntentIndicators.some(indicator => fullUrl.includes(indicator))) {
        trackEvent('intent_classified', { intent: 'medium', method: 'url_analysis' });
        return 'medium';
    }
    ...
    if (demoProgress || calculatorUsed) {
        trackEvent('intent_classified', { intent: 'medium', method: 'previous_activity' });
        return 'medium';
    }
    
    // Check referrer intent
    const referrer = document.referrer.toLowerCase();
    if (referrer.includes('google') && (referrer.includes('chatbot') || referrer.includes('automation'))) {
        trackEvent('intent_classified', { intent: 'medium', method: 'search_intent' });
        return 'medium';
    }
...
function getTrafficSource() {
    ...
    // Industry sites
    if (domain.includes('healthcare') || domain.includes('medical')) return 'healthcare_site';
    if (domain.includes('legal') || domain.includes('law')) return 'legal_site';
    if (domain.includes('shopify') || domain.includes('ecommerce')) return 'ecommerce_site';
...
function getDeviceCategory() {
    ...
    if (width <= 768 || userAgent.includes('mobile') || userAgent.includes('android') || userAgent.includes('iphone')) {
        return 'mobile';
    }
    if (width <= 1024 || userAgent.includes('tablet') || userAgent.includes('ipad')) {
        return 'tablet';
    }
    return 'desktop';
}
...
function updateEngagementScore(eventName, properties) {
...
function personalizeExperience(qualification) {
    switch (qualification) {
        ...
        case 'warm':
            ...
            document.querySelectorAll('[href*="demo"], [href*="calculator"]').forEach(btn => {
    ...
    window.addEventListener('scroll', () => {
        ...
        if (scrollPercent > maxScroll) {
            ...
            scrollDepths.forEach(depth => {
                if (scrollPercent >= depth && !sessionStorage.getItem(`scroll_${depth}`)) {
...
function optimizeUserExperience() {
    const qualification = sessionStorage.getItem('lead_qualification');
    const timeOnPage = Date.now() - sessionData.startTime;
    
    // Show exit intent for engaged users after 2 minutes
    if (timeOnPage > 120000 && sessionData.engagementScore > 3 && !sessionStorage.getItem('exit_intent_shown')) {
...
function getCalculatorEngagement() {
    ...
    const completed = parseInt(localStorage.getItem('calculators_completed') || '0');
    return started > 0 ? completed / started : 0;
}
