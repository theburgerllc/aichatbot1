...
async function calculateROI(params: {
  ...
  const avgHourlyRate = industry === 'legal' ? 75 : industry === 'healthcare' ? 45 : 35;
  
  // Current manual handling time per interaction
  const minutesPerInteraction = industry === 'legal' ? 8 : industry === 'healthcare' ? 6 : 4;
  const totalInteractionsPerMonth = patientsPerDay * workingDaysPerMonth * providers;
  ...
  const monthlySavings = Math.round(timeSavedHours * avgHourlyRate * multiplier.savings);
  const yearlyProjection = monthlySavings * 12;
  const efficiencyGain = Math.round(aiAutomationRate * 100 * multiplier.efficiency);
