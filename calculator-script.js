    ...
    inputs.forEach(input => {
        input.addEventListener('input', (e) => {
            ...
            if (e.target.type === 'range') {
                const valueDisplay = e.target.parentElement.nextElementSibling.querySelector('.current-value');
                if (valueDisplay) {
                    let value = e.target.value;
                    
                    // Format large numbers
                    if (e.target.id.includes('visitors') || e.target.id.includes('case-value')) {
...
function showCalculator(industry) {
    ...
    userEngagement = { inputChanges: 0, timeSpent: 0, resultsViewed: false, emailCaptureShown: false };
...
function calculateHealthcareROI() {
    const providers = parseInt(document.getElementById('healthcare-providers').value);
    const patientsPerDay = parseInt(document.getElementById('healthcare-patients').value);
    ...
    const totalCalls = providers * patientsPerDay * data.callsPerPatient * data.workDaysPerMonth;
    const automatedCalls = totalCalls * data.automationRate;
    const timeSavedMinutes = automatedCalls * data.avgCallDuration;
    const timeSavedHours = timeSavedMinutes / 60;
    const monthlySavings = Math.round(timeSavedHours * hourlyWage * data.roiMultiplier);
    
    // Update primary result with animation
    animateCountUp('healthcare-amount', monthlySavings);
    
    // Update breakdown
    document.getElementById('healthcare-hours').textContent = Math.round(timeSavedHours).toLocaleString();
    document.getElementById('healthcare-calls').textContent = Math.round(automatedCalls).toLocaleString();
    document.getElementById('healthcare-roi-period').textContent = calculateROIPeriod(monthlySavings, 297); // $297/month cost
    
    // Update benchmark comparison
    document.getElementById('healthcare-your-savings').textContent = monthlySavings.toLocaleString();
...
function calculateLegalROI() {
    ...
    const currentConversion = parseInt(document.getElementById('legal-conversion').value);
    ...
    const improvedConversion = Math.min(currentConversion * (1 + data.conversionImprovement), 45);
    const additionalCases = monthlyLeads * ((improvedConversion - currentConversion) / 100);
    const monthlyGrowth = Math.round(additionalCases * caseValue * data.caseValueMultiplier);
    
    // Update primary result
    animateCountUp('legal-amount', monthlyGrowth);
    
    // Update breakdown
    document.getElementById('legal-cases').textContent = additionalCases.toFixed(1);
    document.getElementById('legal-conversion-increase').textContent = `${Math.round(data.conversionImprovement * 100)}%`;
    
    // Update benchmark comparison
    document.getElementById('legal-your-growth').textContent = monthlyGrowth.toLocaleString();
    updateBenchmarkComparison('legal', monthlyGrowth, 48750);
    
    // Update chart
    updateGrowthChart('legal', monthlyLeads, currentConversion, improvedConversion);
...
function calculateEcommerceROI() {
    const monthlyVisitors = parseInt(document.getElementById('ecommerce-visitors').value);
    const abandonmentRate = parseInt(document.getElementById('ecommerce-abandonment').value) / 100;
    ...
    // Update breakdown
    document.getElementById('ecommerce-carts').textContent = recoveredCarts.toLocaleString();
    
    // Update benchmark comparison
    document.getElementById('ecommerce-your-recovery').textContent = monthlyRecovery.toLocaleString();
    updateBenchmarkComparison('ecommerce', monthlyRecovery, 22500);
    
    // Update chart
    updateRecoveryChart('ecommerce', abandonedCarts, recoveredCarts, monthlyRecovery);
...
function updateBenchmarkComparison(industry, yourValue, avgValue) {
    const yourBar = document.querySelector(`#${industry}-calc .your-bar .bar-fill`);
...
function showPostEmailCTAs(industry) {
    ...
    if (ctaSection) {
        ctaSection.style.background = 'linear-gradient(135deg, #F0FDF4 0%, #DCFCE7 100%)';
...
function shareToLinkedIn() {
    ...
    const text = encodeURIComponent(`I just calculated my ROI with AI chatbots - incredible results! Check it out:`);
    window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${url}&text=${text}`, '_blank');
}

function shareToTwitter() {
    const url = encodeURIComponent(window.location.href);
    const text = encodeURIComponent(`Just calculated my ROI with AI chatbots - the results are amazing! 🚀`);
    window.open(`https://twitter.com/intent/tweet?url=${url}&text=${text}`, '_blank');
}

function shareViaEmail() {
    const subject = encodeURIComponent('Check out my AI chatbot ROI calculation');
    const body = encodeURIComponent(`I just used this ROI calculator and the results are impressive! Take a look: ${window.location.href}`);
